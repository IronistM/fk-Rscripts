...
reg_curve <- function(x)
  reg$coefficient[4] * x ^ 3 + reg$coefficient[3] * x ^ 2 + reg$coefficient[2] *
  x + reg$coefficient[1]
...
regslope_curve <-  function(x)
  reg_regslope$coefficient[4] * x ^ 3 + reg_regslope$coefficient[3] * x ^
  2 + reg_regslope$coefficient[2] * x + reg_regslope$coefficient[1]
...
regdd_curve <-  function(x)
  reg_regdd$coefficient[4] * x ^ 3 + reg_regdd$coefficient[3] * x ^ 2 + reg_regdd$coefficient[2] *
  x + reg_regdd$coefficient[1]
...
pos_solve <- function(x)
  regslope_curve(x) + .2
targetcpc <- uniroot.all(function(x)
  pos_solve(x), c(maxcpc_bot, maxcpc_top))
x <-  length(targetcpc)
y <-  targetcpc
if (invalid(targetcpc) == TRUE) {
  targetcpc <-  maxcpc_top
} else{
